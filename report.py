...
class Reporting:
    ...
    def __init__(self, root):
        ...
        enty_ref = ttk.Entry(lbl_frame, textvariable=self.var_ref, width=29, font=("arial", 13, "bold"))
        enty_ref.grid(row=0, column=1)

        btnFetchData=Button(lbl_frame,command=self.fetch_ref,text="Fetch Data",font=("arial",8,"bold"),bg="black",fg="gold",width=8)
    ...
    def update_now(self):
        if self.var_criteria.get() == "":
            ...
            conn=mysql.connector.connect(host="localhost",user="root",password="Sonu@2001",database="th300")
            my_cursor = conn.cursor()
            my_cursor.execute("update room set Name=%s,Email=%s,Criteria=%s,Summary=%s where Ref=%s", (
                self.var_name.get(),
                self.var_email.get(),
                self.var_criteria.get(),
                self.var_summary.get(),
    ...
    def search_m(self):
        conn = mysql.connector.connect(
            host="localhost", username="root", password="Sonu@2001", database="th300")
        my_cursor = conn.cursor()
