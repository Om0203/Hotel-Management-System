...
class Roombooking:
    def __init__(self,root):
        ...
        #=============bill button====================
        btnBill=Button(labelframeleft,text="Bill",command=self.total,font=("arial",8,"bold"),bg="black",fg="gold",width=10)
        ...
        combo_Search=ttk.Combobox(Table_Frame,textvariable=self.search_var,font=("arial",11,"bold"),background="black",foreground="gold",width=24,state="readonly")
    ...
    def add_data(self):
        if self.var_contact.get()=="" or self.var_checkin.get()=="":
            ...
            try:
                conn=mysql.connector.connect(host="localhost",username="root",password="Sonu@2001",database="th300")
...
#update function
    def update(self):
        if self.var_contact.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="Sonu@2001",database="th300")
            my_cursor=conn.cursor()
            my_cursor.execute("Update room2 set check_in=%s,check_out=%s,roomtype=%s,Room=%s,meal=%s,noOfdays=%s where Contact=%s ",(                                                                                                                                       
                                                                                                                                       self.var_checkin.get(),
                                                                                                                                       self.var_checkout.get(),
                                                                                                                                       self.var_roomtype.get(),
                                                                                                                                       self.var_roomavailable.get(),
...
#search system
    def search(self):
        ...
        my_cursor.execute("select * from room2 where "+str(self.search_var.get())+ " LIKE '%"+str(self.txt_search.get())+"%'")
        rows=my_cursor.fetchall()
        if len(rows)!=0:
            self.room_table.delete(*self.room_table.get_children())
            for i in rows:
